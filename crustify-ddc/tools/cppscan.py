...
_CLASS_HEAD = re.compile(
    r"\b(?:class|struct)\s+([A-Za-z_][A-Za-z_0-9]*)"
    r"(?:\s*final)?(?:\s*:\s*[^{;]*)?\s*$"
)


def classes_in(text):
    """Every class/struct DEFINITION as (open_idx, close_idx, name), innermost-last order.

    Needed because an in-class member definition in a header carries NO `Class::` qualifier -- on
    bridge 4 progir.h held 18 of 33 units, and here L3DlOpsScheduler.h, ddc.h, ddc_metadata.h,
    ddl_conversion.h and shuffle.h all define members inside the class body. Without this the
    porter's anchor would not name the class the member belongs to, and a 2-line in-class ctor
    would not be recognised as a ctor.
    """
    san = prepared(text)
    out = []
    for i, c in enumerate(san):
        if c != "{":
            continue
        hs = _head_start(san, i)
        m = _CLASS_HEAD.search(san[hs:i])
        if not m:
            continue
        close = match_brace(san, i)
        if close == -1:
            continue
        out.append((i, close, m.group(1)))
    out.sort(key=lambda t: (t[0], -t[1]))
    return out


def enclosing_class(classes, off):
    """Name of the innermost class/struct whose body contains `off`, else ''."""
    best = None
    for o, c, n in classes:
        if o < off < c and (best is None or o > best[0]):
            best = (o, c, n)
