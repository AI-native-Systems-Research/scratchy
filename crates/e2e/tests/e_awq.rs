...
//! E2E tests for AWQ quantized models.
//!
//! Uses `Qwen/Qwen2.5-0.5B-Instruct-AWQ` (~393 MB).
//! Runs on CPU, NOT MLX.
//!
//! Run with: `cargo test -p vllm-e2e --features e2e --test e_awq -- --ignored --test-threads=1`
...
// ===========================================================================
// Qwen2.5-0.5B-Instruct-AWQ (Qwen2ForCausalLM, AWQ INT4)
// ===========================================================================

#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn test_awq_qwen2_server_starts() {
    let server = TestServer::builder(TestModels::QWEN2_0_5B_AWQ)
        .start()
        .await
        .expect("AWQ server should start");
...
#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn test_awq_qwen2_chat_basic() {
    let server = TestServer::builder(TestModels::QWEN2_0_5B_AWQ)
        .start()
        .await
        .unwrap();
...
#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn test_awq_qwen2_completion_basic() {
    let server = TestServer::builder(TestModels::QWEN2_0_5B_AWQ)
        .start()
        .await
        .unwrap();
...
#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn test_awq_qwen2_max_tokens() {
    let server = TestServer::builder(TestModels::QWEN2_0_5B_AWQ)
        .start()
        .await
        .unwrap();
