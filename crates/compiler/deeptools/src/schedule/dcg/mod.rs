...
//! STAGE 3 of `runDdc`'s pipeline: `DcgManager::runDcgForDlOpsStandalone` (`dcg/dcg_manager/dcg_manager.cpp:449`) — the branch `SchedulerStages.cpp:53-57` picks whenever `dscs_` is non-empty, which is always true for scratchy's input. ⛔ NOT `runDcg`. ⚠️ Its body delegates almost entirely to `dcg_fe/pcfg_gen/` and `dcg_be/`, both OUT of scope, so expect `todo!` naming those rather than an invented PCFG.
//!
