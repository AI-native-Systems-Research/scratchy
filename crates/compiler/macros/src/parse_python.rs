...
//! executable Python for the CI oracle; the `def` is the carrier.
//! Anything else — classes, comprehensions, walrus, star-args,
//! keyword args, `while`, `try`, decorators — is a loud error, the
//! same no-silent-typos property the Rust costume enforces.
...
/// Parse a `.py` carrier file: find the single top-level `def`,
/// parse its body as DSL statements.
pub fn parse_python_file(text: &str, name_span: proc_macro2::Span) -> Result<Ast, String> {
        ...
        match stmt {
            ...
            py::Stmt::FunctionDef(f) => {
                ...
                if !f.decorator_list.is_empty() {
                    return Err("the carrier `def` cannot carry decorators".to_string());
                }
                carrier = Some(f);
            }
            other => {
...
fn parse_stmts(stmts: &[py::Stmt], at: proc_macro2::Span) -> Result<Vec<Stmt>, String> {
    stmts.iter().map(|s| parse_stmt(s, at)).collect()
}
...
mod tests {
    ...
    fn parse(src: &str) -> Ast {
        parse_python_file(src, proc_macro2::Span::call_site()).expect("DSL parse")
    }
    ...
    /// THE prototype claim, as a test: `qwen3.py` (torch costume) and
    /// `qwen3.rs.in` (Rust costume) parse to the same `Ast` and drive
    /// the same pipeline to a byte-identical emit. Runs the whole
    /// classify → shape → CFG → unroll → schedule → codegen chain for
    /// both, under whatever backend feature the test build carries.
    #[test]
    fn python_and_rust_costumes_emit_identically_for_qwen3() {
        use crate::{CompileMode, DEFAULT_DECODER_WORKLOADS, ForwardArgs, parse_carrier};
        ...
        // Rust costume: parse the .rs.in carrier through the normal path.
        let rs_text =
            std::fs::read_to_string(root.join("crates/models/arch/dsl/qwen3.rs.in")).unwrap();
        let rs_file = syn::parse_file(&rs_text).unwrap();
        let carrier_item = rs_file
            .items
            .into_iter()
            .find_map(|item| match item {
                syn::Item::Fn(f) if f.attrs.iter().any(|a| a.path().is_ident("forward")) => {
                    Some(syn::Item::Fn(f))
                }
                _ => None,
            })
            .expect("qwen3.rs.in has a #[forward] fn");
        let carrier = parse_carrier(carrier_item).unwrap();
        let rust_tokens =
            crate::compile_in_dir(&args, &carrier, CompileMode::DECODER, &configs_dir)
                .expect("rust-costume pipeline");

        // Python costume: same arch, same configs, same mode — only
        // the front end differs.
        let py_text =
            std::fs::read_to_string(root.join("crates/models/arch/dsl/qwen3.py")).unwrap();
        let py_ast = parse_python_file(&py_text, proc_macro2::Span::call_site())
            .expect("python-costume parse");
        ...
        let mut rust_rendered = String::with_capacity(1 << 20);
        let mut py_rendered = String::with_capacity(1 << 20);
        crate::render_tokens(rust_tokens, &mut rust_rendered);
        ...
        // this check, an unscoped run emits two EMPTY token streams
        // and the comparison below passes having verified nothing.
        assert!(
            rust_rendered.len() > 100_000,
        ...
        if rust_rendered != py_rendered {
            let r = rust_rendered.as_bytes();
            let p = py_rendered.as_bytes();
            let first = r
                .iter()
                .zip(p.iter())
                .position(|(a, b)| a != b)
                .unwrap_or_else(|| r.len().min(p.len()));
            panic!(
                "costumes diverge: rust={}B py={}B first-diff at byte {first}\n\
                 rust: {:?}\n  py: {:?}",
                r.len(),
                p.len(),
                &rust_rendered[first.saturating_sub(60)..(first + 120).min(r.len())],
