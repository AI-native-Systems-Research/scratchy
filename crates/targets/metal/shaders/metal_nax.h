...
// Upstream steel utils export this inside mlx::steel; our preamble
// defines it in `metal` — alias so the verbatim-vendored body resolves.
template <typename T>
using pointer_element_t = metal::pointer_element_t<T>;
...
METAL_FUNC constexpr auto operator+(integral_constant<T,tv>, integral_constant<U,uv>) {
  constexpr auto r = tv + uv; return integral_constant<metal::remove_addrspace_t<decltype(r)>, r>{};
}
template <typename T, T tv, typename U, U uv>
METAL_FUNC constexpr auto operator*(integral_constant<T,tv>, integral_constant<U,uv>) {
  constexpr auto r = tv * uv; return integral_constant<metal::remove_addrspace_t<decltype(r)>, r>{};
...
// ─────────────────────────────────────────────────────────────────────────────
// NAX tile machinery — vendored VERBATIM from MLX HEAD
// mlx/backend/metal/kernels/steel/gemm/nax.h (post "Nax Refactor"
// mlx#3271 — the pre-refactor vendored copy ran 10.1/10.9 TFLOPS vs
// the refactored 14.1/13.1 on identical Gemma4 MLP qmm shapes).
// Local edits: none inside this block; the preamble above supplies
// the steel defines/integral_constant/pointer_element utilities the
// upstream header pulls from the steel tree.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// QuantizedBlockLoader + dequantize — vendored VERBATIM from
// mlx/backend/metal/kernels/quantized_nax.h so the NAX qmm kernel uses
// MLX's exact W-loader (the hand-inlined loader was ~1.33x slower at
// every M=4096 prefill shape even though it produced bit-identical
// output; only the general gs>=64 loader is needed — the dispatcher
// gates gs==32 to the non-NAX path).
// ─────────────────────────────────────────────────────────────────────────────
...
// Tdst = threadgroup/dequant type (== T_act); Tsc = device scale/bias type
...
// Plain dense block loader (non-quantized) — vendored from mlx
// `steel/gemm/loader.h`. Stages a BROWS×BCOLS device tile into a
// BROWS×dst_ld threadgroup tile. Used by `gemm_t_nax_impl` (plain bf16
// NAX GEMM) the same way QuantizedBlockLoader is used by the int4 path.
template <
    typename T,
    short BROWS,
    short BCOLS,
...
struct QuantizedBlockLoader {
  ...
  QuantizedBlockLoader(
      ...
      const device Tsc* scales_,
      const device Tsc* biases_,
      const int src_ld_,
      threadgroup T* dst_,
