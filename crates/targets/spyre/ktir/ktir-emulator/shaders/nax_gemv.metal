...
// B (weight) operand element type — the same switch the matmul kernels use. Default
// f32; KTIR_B_F16=1 reads B as `half` directly (the KTIR_F16_WEIGHTS path — half the
// bytes streamed). Bit-identical either way: B is folded to `half` before the
// multiply regardless, so only the device load width changes.
#ifndef KTIR_B_F16
#define KTIR_B_F16 0
#endif
#if KTIR_B_F16
...
[[kernel]] void nax_gemv(
    ...
    device const ktir_bt* b_in [[buffer(1)]], // B: K x N, or N x K (transpose_b)
