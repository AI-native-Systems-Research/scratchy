...
// SGS_N (simdgroup cols per threadgroup) is overridable so the host can compile a
// NARROW-N variant (SGS_N=1 -> TG_N=64) for decode. At m=1 the grid is [N/TG_N, 1],
// so the default TG_N=256 dispatches only 32 threadgroups for an N=8192 FFN and 8 for
// an N=2048 projection — fewer than the GPU has cores. Narrowing TG_N multiplies the
// threadgroup count by 4 at the SAME total thread count, which is a scheduling change,
// not a math change: every simdgroup still computes the same SG_M x SG_N sub-block.
#ifndef KTIR_SGS_N
#define KTIR_SGS_N 4
#endif
constant constexpr uint SGS_N  = KTIR_SGS_N;  // simdgroup cols per threadgroup
...
#define STAGE_PANEL(buf, kc)                                                    \
    do {                                                                       \
        ...
        for (uint ch = tid; ch < arows * CPR; ch += TG_THREADS) {              \
            uint r  = ch / CPR;                                                 \
    ...
    // Rows of the A tile past the real M are zero for EVERY K-step, so zero both
    // buffers' padding once here and stage only `arows` below. At decode (M=1, TG_M=128)
    // that is 4 real chunks per step instead of 512, on all nk steps; the fragment math
    // still reads a full TG_M-row tile and stays bit-identical.
    const uint arows = min(TG_M, M - tm0);
    for (uint i = tid; i < 2u * TG_M * BK; i += TG_THREADS) {
        if ((i % (TG_M * BK)) / BK >= arows) { a_tg[i] = half(0); }
    }
