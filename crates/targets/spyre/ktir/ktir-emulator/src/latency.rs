...
mod tests {
    ...
    fn zero_category_costs_nothing() {
        ...
        t.record_op(0, OpKind::ScfYield, LatencyCategory::Zero, &None, &[]);
        let c = &t.counters()[&0];
    ...
    fn vector_int_charges_elements() {
        ...
        t.record_op(0, OpKind::ArithAddi, LatencyCategory::ComputeInt, &res, &[]);
        let c = &t.counters()[&0];
        assert!((c.compute_cycles - 2.0).abs() < 1e-9);
        assert_eq!(c.total_flops, 128.0);
    }
    ...
    fn trace_records_per_op_entries() {
        ...
        assert_eq!(trace[0].op_type, OpKind::ArithAddf);
