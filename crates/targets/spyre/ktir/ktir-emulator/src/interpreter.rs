...
use crate::ir::{IRFunction, Operation, Scalar, Ssa, Value};
...
fn profile_class(op_type: OpKind) -> &'static str {
    match op_type {
        OpKind::LinalgMatmul | OpKind::LinalgBatchMatmul => "matmul",
        OpKind::KtdpLoad | OpKind::KtdpStore => "load/store",
        other => match other.dialect() {
            Dialect::Linalg => "linalg-other",
            Dialect::Arith => "arith",
            Dialect::Math => "math",
            Dialect::Ktdp => "ktdp-other",
            Dialect::Scf => "scf",
            Dialect::Tensor => "tensor",
            Dialect::Func | Dialect::Region => "other",
        },
    }
}
...
pub fn execute_op(
    ...
    if let Some(tracker) = env.tracker {
        ...
        let operands: Vec<Option<Value>> = op
            .operands
            .iter()
            .map(|&v| ctx.get_value(v).ok().cloned())
            .collect();
        ...
        && names.len() > 1
    {
        let Some(Value::Tuple(vals)) = &produced else {
            return Err(format!(
                "op {:?} has {} results but did not produce a tuple",
                op.op_type,
                names.len()
            ));
        };
        if vals.len() != names.len() {
            return Err(format!(
                "op {:?}: {} results but produced {} values",
                op.op_type,
                names.len(),
                vals.len()
            ));
    ...
    if let Some(res) = op.result {
        match produced {
            Some(val) => {
                ...
                if let Value::Tile(t) = &val {
                    ctx.track_lx_tile(res, t)?;
                }
                ctx.set_value(res, val.clone());
                return Ok(Some(val));
            }
            None => {
                return Err(format!(
                    "op {:?} has result %{} but produced no value",
                    op.op_type, res.0
                ));
            }
        }
...
fn try_fuse_matmul_epilogue(
    ...
    let mm = &ops[i];
    if mm.op_type != OpKind::LinalgMatmul || mm.operands.len() != 2 {
        return Ok(None);
    }
    let Some(cval) = mm.result else {
        return Ok(None);
    };
    let Some(ep) = ops.get(i + 1) else {
        return Ok(None);
    };
    let Some(dval) = ep.result else {
        return Ok(None);
    };
    ...
    let eval = if ep.operands[0] == cval {
        ep.operands[1]
    } else if ep.operands[1] == cval && commutative {
        ep.operands[0]
    } else {
        return Ok(None);
    };
    ...
    let reused = ops[i + 2..].iter().any(|o| o.operands.contains(&cval))
        || ops[i + 1].operands.iter().filter(|&&x| x == cval).count() > 1;
    if reused {
        return Ok(None);
    }
    ...
    ctx.track_lx(dval, tile.size_bytes() as i64)?;
    ctx.set_value(dval, crate::ir::Value::Tile(tile));
    Ok(Some(2))
}

/// Borrow an SSA value as a `Tile`, or `Err` if it isn't one.
#[cfg(metal)]
fn as_tile(ctx: &CoreContext, v: Ssa) -> Result<&crate::tile::Tile, String> {
    match ctx.get_value(v)? {
        crate::ir::Value::Tile(t) => Ok(t),
        _ => Err(format!("fuse: %{} is not a tile", v.0)),
    }
}
...
pub(crate) fn execute_function(
    ...
    execute_function_filtered(func, args, None)
}

/// Like [`execute_function`] but reads back ONLY the given tensor args, skipping
...
pub(crate) fn execute_function_outputs(
    ...
    let wanted: std::collections::HashSet<Ssa> = outputs.iter().copied().collect();
...
fn execute_function_filtered(
    func: &IRFunction<'static>,
    args: &[(Ssa, Arg)],
    wanted: Option<&std::collections::HashSet<Ssa>>,
) -> Result<HashMap<Ssa, Output>, String> {
    ...
    #[cfg(metal)]
    if let Ok(out) = execute_function_gpu(func, args) {
        return Ok(out);
    }
...
) -> Result<(HashMap<Ssa, Output>, crate::latency::LatencyReport), String> {
...
pub fn execute_function_seeded(
    ...
    let grid = GridExecutor::new(func.grid);
    let dispatch = Dispatch::shared();
    let input_ptrs: Vec<(Ssa, Value)> = scalars
        .iter()
        .map(|(v, s)| (*v, Value::Scalar(*s)))
        .collect();
...
/// Tensor read-back metadata: `(arg, stick, n_elements, shape, dtype)`.
pub type TensorMeta = (Ssa, i64, usize, Vec<usize>, DType);
...
fn marshal_inputs(
    ...
    fn place(
        ...
        input_ptrs.push((arg, Value::Index(elem_idx)));
        tensor_meta.push((arg, stick, shape.iter().product(), shape.to_vec(), dtype));
    }
    for &(name, ref arg) in args {
        match arg {
            ...
            Arg::Scalar(s) => input_ptrs.push((name, Value::Scalar(*s))),
        }
    }
