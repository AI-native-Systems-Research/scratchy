...
use crate::ir::{Attr, IRFunction, IRModule, Operation, Ssa};
...
pub(crate) fn collect_view_shapes(
    ops: &[Operation],
...
/// the FA golden uses).
pub(crate) fn apply_attention_rewrites<'a>(a: &'a Arena, module: &mut IRModule<'a>) {
    ...
    // it re-tiles here. Runs BEFORE the attention rewrites: those recognise whole-tensor score/
    // context blocks, and a contraction inside one must already be in its tiled form when they look.
    let n_mm = ktir_optimizer::matmul_tile::apply_matmul_tiling(a, module);
    if n_mm > 0 && std::env::var_os("KTIR_REWRITE_VERBOSE").is_some() {
        eprintln!("[ktir-optimizer] matmul LX tiling: {n_mm} contraction(s)");
    }
    ...
    // re-rolled form.
    let n_head = ktir_optimizer::head_rewrite::apply_head_rewrite(a, module, |scores_bytes| {
        ktir_optimizer::fusion::attention_needs_flash(scores_bytes, budget)
    });
    let n_flash = ktir_optimizer::flash_attn::apply_flash_attention(a, module, |scores_bytes| {
        ktir_optimizer::fusion::attention_needs_flash(scores_bytes, budget)
    });
    ...
    let n_coalesce = if std::env::var_os("KTIR_NO_TILE_COALESCE").is_some() {
        ...
        ktir_optimizer::tile_coalesce::apply_tile_coalesce(
            a,
            module,
...
pub(crate) fn execute_segmented(
    ...
    let a = Arena::global();
    let mut owned = module_of(funcs);
    apply_attention_rewrites(a, &mut owned);
    let module = &owned;
    ...
    for &(tid, ref arg) in args {
        let (data, dtype) = match arg {
            ...
            Arg::Scalar(_) => {
                return Err(format!(
                    "scalar arg t{tid} unsupported in execute_segmented"
                ));
            }
        };
    ...
    for seg in &segments {
        ...
        match seg {
            ...
            Segment::Fused(fs) => {
                let mut call_args: Vec<(Ssa, Arg)> = Vec::new();
                let mut want: Vec<(Ssa, u64)> = Vec::new();
                // The arg -> tensor mapping travels WITH the segment (`fs.args`); it
                // used to be recovered by parsing the argument's `%t<id>_ptr` name.
                for &(arg, tid) in &fs.args {
                    ...
                    call_args.push((arg, Arg::Tensor { data, shape, dtype }));
                }
                let want_args: Vec<Ssa> = want.iter().map(|&(a, _)| a).collect();
                let out = execute_function_outputs(&fs.func, &call_args, &want_args)?;
                for &(arg, tid) in &want {
                    let o = out
                        .get(&arg)
                        .ok_or_else(|| format!("fused segment output t{tid} not read back"))?;
            ...
            Segment::Native(node) => {
                ...
                let out = execute_function(module.get_function(&node.func)?, &call_args)?;
                for &(name, tid) in &out_ids {
                    let o = out
                        .get(&name)
                        .ok_or_else(|| format!("native attn output t{tid} not read back"))?;
                    buf.insert(tid, (o.data.clone(), o.dtype));
                }
            }
        }
    ...
    // Return the requested outputs, keyed by tensor id.
    let mut result: HashMap<u64, Output> = HashMap::new();
    for &tid in outputs {
        let (data, dtype) = buf
            .get(&tid)
            .cloned()
...
/// Collect `funcs` into an [`IRModule`] keyed by function name — the form the
/// optimizer's whole-program passes and the [`ProgramSpec`]'s node names index.
/// A launch group arrives as a `&[IRFunction]`; this is the only place the two
/// shapes meet, and nothing is copied but the name keys.
pub(crate) fn module_of(funcs: &[IRFunction<'static>]) -> IRModule<'static> {
