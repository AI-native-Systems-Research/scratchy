...
impl HBMSimulator {
    ...
    /// As [`write_bytes`](Self::write_bytes), but TAKING the buffer: when the write exactly
    /// covers an existing allocation from its base, that buffer REPLACES it rather than
    /// being copied into it. Loading a checkpoint is one such write per tensor, so this is
    /// a whole pass over the weights that does not happen.
    ///
    /// The length equality is what makes the swap safe: [`find_allocation`] derives an
    /// allocation's extent from its buffer's length, so installing a buffer of a different
    /// length would move the boundary every interior access is resolved against. Anything
    /// else falls back to the copying path.
    /// The length of the allocation based EXACTLY at `byte_addr` — the extent a view built
    /// over that tensor's own base may not exceed. `None` when `byte_addr` is not a base.
    ///
    /// Deliberately NOT [`find_allocation`]: that resolves an INTERIOR pointer to whichever
    /// allocation contains it, and [`write_bytes`] grows an allocation in place
    /// (`buf.resize`) when a write runs past its initial size, so a grown allocation can
    /// swallow the base of the one the bump allocator placed after it. Attributing an
    /// interior pointer under those conditions can name the wrong tensor, so a bounds check
    /// built on it would report defects that are artifacts of the lookup.
    pub fn allocation_len_at_base(&self, byte_addr: i64) -> Option<usize> {
        self.allocations.get(&byte_addr).map(Vec::len)
    }

    pub fn write_bytes_owned(&mut self, byte_addr: i64, data: Vec<u8>) {
        match self.allocations.get(&byte_addr) {
            Some(buf) if buf.len() == data.len() => {
                self.allocations.insert(byte_addr, data);
            }
            _ => write_bytes(&mut self.allocations, byte_addr, &data),
        }
    }
}
