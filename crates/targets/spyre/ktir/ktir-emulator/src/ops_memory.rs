...
pub fn register(d: &mut Dispatch) {
    d.register(OpKind::KtdpLoad, LatencyCategory::Memory, load);
    d.register(OpKind::KtdpStore, LatencyCategory::Memory, store);
}
...
fn lower_to_box(aset: &AffineSet) -> Option<BoxSet> {
    // The symbolic bounds are resolved to integers below, so nothing built here
    // outlives the call — a scratch arena holds them for its duration.
    let ar = crate::arena::Arena::new();
    let sym = SymBoxSet::try_from_affine_set(&ar, aset)?;
    if !sym.is_concrete() {
        return None;
    }
...
fn decode_gather(raw: &[u8], offsets: &[i64], dtype: DType) -> Vec<f32> {
    ...
    match dtype {
        // One byte per element, widened on read — the packed device form.
        DType::Fp8E4m3 => {
            for &o in offsets {
                let off = o as usize;
                let v = match raw.get(off) {
                    Some(&b) => crate::codec::e4m3_to_f32(b),
                    None => 0.0,
                };
                out.push(v);
            }
