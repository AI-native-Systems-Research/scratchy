...
use crate::ir::{Attr, Operation, Ssa, TileFuture, Value};
...
/// A result type is a shape plus an element type; both are hashed structurally.
fn hash_rtype(t: &Option<IrType<'_>>, h: &mut std::collections::hash_map::DefaultHasher) {
    use std::hash::Hash;
    match t {
        None => 0u8.hash(h),
        Some(t) => {
...
thread_local! {
    static DIES_AT_CACHE: std::cell::RefCell<HashMap<u64, Rc<Vec<Vec<Ssa>>>>> =
        std::cell::RefCell::new(HashMap::new());
}

/// Count every operand use across all ops and nested regions — the Rust port of
/// Python's `KTIRParser._build_use_counts`. Drives consume-on-last-use (#134): a
/// value with count 1 is freed at its single use. Counts `op.operands`, recursing
/// into regions; values named by attributes are intentionally NOT counted,
/// matching Python.
fn build_use_counts(ops: &[Operation]) -> std::collections::HashMap<Ssa, usize> {
    fn walk(ops: &[Operation], counts: &mut std::collections::HashMap<Ssa, usize>) {
        for op in ops {
...
/// Memoized [`compute_dies_at`] keyed by `key` ([`plan_key`]). The dead values are
/// already identities — the analysis used to hand back NAMES that then had to be
/// interned before the reclaim could index a slot — so the liveness reclaim runs
/// `forget` per dead value with no lookup at all.
fn cached_dies_at(ops: &[Operation], key: u64) -> Rc<Vec<Vec<Ssa>>> {
    DIES_AT_CACHE.with(|c| {
        if let Some(v) = c.borrow().get(&key) {
            return Rc::clone(v);
        }
        let v = Rc::new(compute_dies_at(ops));
...
thread_local! {
    static MATMUL_SCHED_CACHE: std::cell::RefCell<
        HashMap<u64, Rc<HashMap<Ssa, crate::metal::MatmulLoopInfo>>>,
    > = std::cell::RefCell::new(HashMap::new());
...
fn cached_matmul_schedule(
    ...
    key: u64,
) -> Rc<HashMap<Ssa, crate::metal::MatmulLoopInfo>> {
...
#[cfg(metal)]
fn cached_map_fusion_plan(ops: &[Operation<'static>], key: u64) -> Rc<MapPlan> {
...
pub fn is_comm_op(op_type: OpKind) -> bool {
    matches!(op_type, OpKind::KtdpReduce | OpKind::KtdpInterTileReduce)
}
...
struct InterTileReduce {
    ...
    identity: Tile,
    /// `^bb0(%lhs, %rhs)` block arguments + the combiner region. Run op by op to
    /// fold two tiles, capturing the `ktdp.yield_reduced` terminator's value.
    lhs: Ssa,
    rhs: Ssa,
    combiner: &'static [Operation<'static>],
...
impl InterTileReduce {
    fn new(op: &Operation<'static>, ctx: &CoreContext, num_cores: usize) -> Result<Self, String> {
        ...
        let consumer_set = match op.attr(AttrKey::ConsumerTilesPerGroup) {
        ...
        let combiner: &'static [Operation<'static>] = op.regions.first().copied().unwrap_or(&[]);
        let bb0 = combiner
            .iter()
            .find(|o| o.op_type == OpKind::RegionBb0Args)
            .and_then(|o| match o.attr(AttrKey::Names) {
                Some(Attr::Ssas(names)) => Some(*names),
                _ => None,
            })
        ...
        let result_shape = match op.attr(AttrKey::ResultShape) {
    ...
    fn combine(
        ...
        ctx.push_scope();
        let out = (|| {
            ctx.set_value(self.lhs, Value::Tile(lhs.clone()));
            ctx.set_value(self.rhs, Value::Tile(rhs.clone()));
            // The `ktdp.yield_reduced` terminator RETURNS the folded tile (as
            // `scf.yield` does); it used to park it under a reserved scope key.
            let mut yielded: Option<Value> = None;
            for o in self.combiner {
                if o.op_type == OpKind::RegionBb0Args {
                    continue;
                }
                let produced = execute_op(o, ctx, env)?;
                if o.op_type == OpKind::KtdpYieldReduced {
                    yielded = produced;
                }
            }
            match yielded {
                Some(Value::Tile(t)) => Ok(t),
                other => Err(format!(
...
struct RegionCommDriver {
    ...
    inner: Option<(Box<dyn CommOp>, Option<Ssa>)>,
    /// The result value of the outermost scf op, to bind its final value.
    root_result: Option<Ssa>,
...
impl RegionCommDriver {
    ...
    fn new(idx: usize, ops: &[Operation<'static>], ctx: &mut CoreContext) -> Result<Self, String> {
        let op = &ops[idx];
        let root_result = op.result;
    ...
    fn op_at<'a>(ops: &'a [Operation<'static>], path: &[usize]) -> &'a Operation<'static> {
...
fn build_frame(
    ...
    match op.op_type {
        ...
        other => Err(format!(
            "RegionCommDriver: not a control-flow op: {other:?}"
        )),
    }
}
...
fn frame_body<'a>(
    frame: &RegionFrame,
    ops: &'a [Operation<'static>],
) -> (&'a [Operation<'static>], usize) {
    match frame {
        RegionFrame::For { path, .. } => {
            let op = RegionCommDriver::op_at(ops, path);
            (op.regions.first().copied().unwrap_or(&[]), 0)
        }
        RegionFrame::If { path, which, .. } => {
            let op = RegionCommDriver::op_at(ops, path);
            match which {
                Some(r) => (op.regions[*r], *r),
                None => (&[], 0),
            }
        }
    }
        ...
        loop {
            ...
            if is_comm_op(body_op.op_type) {
                ...
                match comm.step(ctx, env, None)? {
                    CommStep::Recv(req) => {
                        self.inner = Some((comm, body_op.result));
                        return Ok(FrameStep::Suspend(req));
                    }
                    CommStep::Done(val) => {
                        bind_result(ctx, body_op.result, *val)?;
                    }
                }
            } else if (body_op.op_type == OpKind::ScfFor || body_op.op_type == OpKind::ScfIf)
                && body_op.regions.iter().any(|r| region_has_comm(r))
            {
                ...
                let is_yield = body_op.op_type == OpKind::ScfYield;
...
fn finalize_for(iter_arg_names: &[Ssa], current_values: &[Value]) -> Option<Value> {
...
struct CoreRunner {
    ...
    active: Option<(Box<dyn CommOp>, Option<Ssa>)>,
...
impl CoreRunner {
    ...
    fn step(
        ...
        if let Some((comm, result_name)) = &mut self.active {
            match comm.step(&mut self.ctx, env, incoming.take())? {
                ...
                CommStep::Done(val) => {
                    let name = *result_name;
                    self.active = None;
                    bind_result(&mut self.ctx, name, *val)?;
                }
            }
        }
        ...
        #[cfg(metal)]
        let matmul_sched: Rc<HashMap<Ssa, crate::metal::MatmulLoopInfo>> = if gpu_offload {
        ...
        while self.op_idx < ops.len() {
            ...
            if is_comm_op(op.op_type) {
                ...
                match comm.step(&mut self.ctx, env, None)? {
                    CommStep::Recv(req) => {
                        self.active = Some((comm, op.result));
                        return Ok(Poll::Block(req.src));
                    }
                    CommStep::Done(val) => bind_result(&mut self.ctx, op.result, *val)?,
                }
            } else if (op.op_type == OpKind::ScfFor || op.op_type == OpKind::ScfIf)
                && op.regions.iter().any(|r| region_has_comm(r))
            {
                ...
                // Name the OP in any failure: the op-level errors below carry an SSA and a shape,
                // neither of which says which operation built it.
                execute_op(op, &mut self.ctx, env).map_err(|e| format!("{:?}: {e}", op.op_type))?;
            }
...
fn compute_dies_at(ops: &[Operation]) -> Vec<Vec<Ssa>> {
    // Recursively record the highest TOP-LEVEL index at which each value is used.
    // Uses come from operands AND from the attributes that NAME values (`iter_var`
    // / `iter_args` / `result_names` / a dynamic `sizes_dyn`) — missing those frees
    // a value too early. Those used to be recognized by a leading `%` on a string
    // attribute, which both over-matched and under-matched; an [`Attr::Ssas`] IS
    // the list of values the attribute names, so the walk is exact.
    fn note_uses(op: &Operation, top_idx: usize, last_use: &mut HashMap<Ssa, usize>) {
        for &operand in op.operands {
            last_use.insert(operand, top_idx);
        }
        for (_, attr) in op.attributes {
            if let crate::ir::Attr::Ssas(xs) = attr {
                for &x in *xs {
                    last_use.insert(x, top_idx);
                }
            }
        }
        for region in op.regions {
...
fn bind_result(ctx: &mut CoreContext, name: Option<Ssa>, val: Option<Value>) -> Result<(), String> {
    if let (Some(name), Some(val)) = (name, val) {
        if let Value::Tile(t) = &val {
            // Name the tile SHAPE in an LX refusal — the charge is a product, so only the shape
            // says which emitted tile is too big.
            ctx.track_lx(name, t.size_bytes() as i64)
                .map_err(|e| format!("{e} (tile {:?})", t.shape))?;
        }
...
pub fn execute_with_communication(
    ...
    let dies_at = cached_dies_at(ops, plan_key);
        ...
        && !ops.iter().any(|o| is_comm_op(o.op_type))
    {
        ...
        let ops_ptr = ops as *const [Operation<'static>];
...
pub fn execute_function_single_tile(
    ...
    let dies_at = cached_dies_at(ops, pk);
    let mut ctx = CoreContext::new(
        tile,
