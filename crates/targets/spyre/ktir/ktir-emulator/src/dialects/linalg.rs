...
//!
//! `linalg.yield %v` RETURNS `%v`, as `scf.yield` does; [`run_region`] drives the
//! body op by op and keeps the terminator's value. It used to be parked in the
//! value table under a reserved NAME (`"__linalg_yield__"`), chosen never to
//! collide with a real SSA name — a spelling an [`Ssa`] cannot express.
//!
//! ## N-dimensional tiles
//!
...
pub fn register(d: &mut Dispatch) {
    ...
    d.register(OpKind::LinalgMatmul, LatencyCategory::ComputeFloat, matmul);
    d.register(
    ...
    d.register(OpKind::LinalgReduce, LatencyCategory::Zero, reduce);
    d.register(OpKind::LinalgTranspose, LatencyCategory::Zero, transpose);
    d.register(OpKind::LinalgBroadcast, LatencyCategory::Zero, broadcast);
    d.register(OpKind::LinalgFill, LatencyCategory::Zero, fill);
    d.register(OpKind::LinalgIndex, LatencyCategory::Zero, index);
    d.register(OpKind::LinalgYield, LatencyCategory::Zero, yield_op);
    // Elementwise named ops: `linalg.add/sub/mul/div/max/min ins(%a, %b) outs(%c)`.
    d.register(
        OpKind::LinalgAdd,
        LatencyCategory::ComputeFloat,
        |o, c, _| elementwise(o, c, |a, b| a + b),
    );
    d.register(
        OpKind::LinalgSub,
        LatencyCategory::ComputeFloat,
    ...
    d.register(
        OpKind::LinalgMax,
        LatencyCategory::ComputeFloat,
        |o, c, _| elementwise(o, c, f32::max),
    );
    d.register(
        OpKind::LinalgMin,
        LatencyCategory::ComputeFloat,
        |o, c, _| elementwise(o, c, f32::min),
    );
}
...
fn broadcast(
    ...
    let mut dims = int_list_attr(op, AttrKey::Dimensions)
        .map(<[i64]>::to_vec)
        .unwrap_or_default();
    dims.sort_unstable();
...
fn transpose(
    ...
    let perm = int_list_attr(op, AttrKey::Permutation)
...
fn reduce(
    ...
    // Resolve the combiner region (capturing its bb0 block arguments).
    let (bb0_names, body_ops) = resolve_region_body(op);

    // Combiner: the explicit form has it as the region's first non-yield op;
    // the shorthand names it in `reduce_fn`. Default to arith.addf.
    let reduce_fn = match op.attr(AttrKey::ReduceFn) {
        Some(Attr::Op(k)) => Some(*k),
        _ => None,
    }
    .or_else(|| {
        body_ops
    ...
    .unwrap_or(OpKind::ArithAddf);

    // SHORTHAND (no region): fold `reduce_fn` directly. This used to synthesize a
    // one-op region over invented block-argument NAMES (`"__reduce_in__"`,
    // `"__reduce_acc__"`) — identities nothing minted, so there is nothing to
    // invent them as. The fold is exactly what that synthesized region computed.
    let shorthand = if body_ops.is_empty() {
        Some(reduce_combiner(reduce_fn).ok_or_else(|| {
            format!("linalg.reduce: shorthand combiner {reduce_fn:?} is not a known f32 fold")
        })?)
    } else {
        None
    };
    ...
    let dims: Option<Vec<usize>> = match op.attr(AttrKey::Dim) {
        Some(Attr::Int(d)) => Some(vec![*d as usize]),
        _ => match op.attr(AttrKey::Dimensions) {
    ...
    let fast_combine = if let Some(f) = shorthand {
        Some(f)
    } else if body_ops.len() == 2
        && body_ops[1].op_type == OpKind::LinalgYield
        && body_ops[0].operands.len() == 2
        && body_ops[0].operands.iter().all(|o| bb0_names.contains(o))
    {
    ...
    let outs_var = match op.attr(AttrKey::OutsVar) {
        Some(Attr::Ssas(v)) => v.first().copied(),
        _ => None,
    };
    if let Some(outs_var) = outs_var
        && let Ok(Value::Tile(outs_tile)) = ctx.get_value(outs_var)
    {
        ...
        if reduced_tile.shape == outs_tile.shape {
            // The shorthand has no region to run, so it folds the pair directly —
            // exactly what the region it used to synthesize computed.
            let combined = match shorthand {
                Some(f) => {
                    let mut data: Vec<f32> = reduced_tile
                        .as_f32()
                        .iter()
                        .zip(outs_tile.as_f32().iter())
                        .map(|(&a, &b)| f(a, b))
                        .collect();
                    crate::codec::round_to_dtype(&mut data, reduced_tile.dtype);
                    Value::Tile(Tile::compute(
                        data,
                        reduced_tile.dtype,
                        reduced_tile.shape.clone(),
                    ))
                }
                None => run_combiner(&bb0_names, &body_ops, reduced_tile, outs_tile, ctx, env)?,
            };
    ...
    // reference it by the outs value. Bind both so either reference resolves.
    if let Some(outs_var) = outs_var {
...
fn reduce_combiner(op_name: OpKind) -> Option<fn(f32, f32) -> f32> {
    Some(match op_name {
        OpKind::ArithAddf => |a, b| a + b,
        OpKind::ArithMulf => |a, b| a * b,
        OpKind::ArithMaxnumf => f32::max,
        OpKind::ArithMinnumf => f32::min,
        // `maxf`/`minf` are the arith dialect's aliases for `maximumf`/`minimumf`
        // (see the registration in `dialects::arith`), so they fold identically.
        OpKind::ArithMaximumf | OpKind::ArithMaxf => |a: f32, b: f32| {
        ...
        OpKind::ArithMinimumf | OpKind::ArithMinf => |a: f32, b: f32| {
            if a.is_nan() || b.is_nan() {
                f32::NAN
            } else if a <= b {
                a
...
fn generic(
    ...
    let n_ins = match op.attr(AttrKey::NIns) {
    ...
    // The iteration shape `linalg.index` reads, for this region only.
    ctx.push_index_shape(out_shape.clone());
...
fn index(
    ...
    let dim = match op.attr(AttrKey::Dim) {
        Some(Attr::Int(d)) => *d as usize,
        _ => 0,
    };
    let out_shape: Vec<usize> = ctx
        .index_shape()
        .ok_or("linalg.index: no enclosing linalg.generic iteration shape")?
        .to_vec();
...
/// `linalg.yield %v` — hand `%v` back to the enclosing region driver (see
/// [`run_region`]), as `scf.yield` does.
fn yield_op(
    op: &Operation,
    ctx: &mut CoreContext,
    _env: &ExecutionEnv,
...
/// Run `body_ops` in a fresh scope after `bind` populates the block args, and
/// return the value its `linalg.yield` terminator produced. Owns the `push_scope`
...
fn run_region(
    ...
    let outcome = (|| {
        ...
        for o in body_ops {
            ...
            if o.op_type == OpKind::LinalgYield {
                yielded = produced;
            }
        }
        Ok(yielded)
...
fn resolve_region_body(op: &Operation<'static>) -> (Vec<Ssa>, Vec<Operation<'static>>) {
    let region: &[Operation<'static>] = op.regions.first().copied().unwrap_or(&[]);
    let body_ops: Vec<Operation<'static>> = region
        .iter()
        .filter(|o| o.op_type != OpKind::RegionBb0Args)
        .cloned()
        .collect();

    if let Some(Attr::Ssas(names)) = op.attr(AttrKey::Bb0Names) {
        return (names.to_vec(), body_ops);
    }
    ...
    if let Some(Attr::Ssas(names)) = region
        .iter()
        .find(|o| o.op_type == OpKind::RegionBb0Args)
        .and_then(|o| o.attr(AttrKey::Names))
    {
        return (names.to_vec(), body_ops);
    }
    if let Some(first) = body_ops.first() {
        return (first.operands.to_vec(), body_ops);
    }
    (Vec::new(), Vec::new())
}
...
fn int_list_attr<'a>(op: &Operation<'a>, key: AttrKey) -> Option<&'a [i64]> {
    match op.attr(key) {
