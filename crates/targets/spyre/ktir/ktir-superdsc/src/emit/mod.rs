...
/// `gathered: true` gives every core the SAME base instead of its own WORK-SLICE CORNER.
///
/// ⛔ IT IS NOT A CALLER'S CHOICE. The single call site derives it from
/// [`OpSpec::indirect_index_pair`](crate::superdsc_opspec::OpSpec::indirect_index_pair) — "is this
/// operand the one this op's own `Role::KernelIdx` arg indexes?" — so it is `false` for every operand of
/// every op that carries no index arg, and `false` is byte-identical to the former no-argument form.
///
/// `true` is what "gathered" MEANS: the row a core reads comes from the index, not from a corner, and
/// dbo reduces the value tensor's per-core starts to one `min` before deriving any address
/// (`computeGatherMetadata`).
...
fn per_core_addr(
    ...
    //
    // ⛔ THE GATE IS THE ROLE, AND A `Role::KernelIdx` OPERAND EXISTS ONLY BECAUSE
    // `OpSpec::attach_indirect_index` PUT ONE THERE. So every operand of every op that does not gather
    // enters the `match` below exactly as it always did — the skip is unreachable without a gather
    // rather than merely inert for one.
    ...
    for c in 0..cores {
        ...
        //
        // ⛔ A GATHERED OPERAND HAS NO CORNER AT ALL, so the whole fold below is SKIPPED for it rather
        // than neutered inside itself: `gathered` is true only for the operand this op's own
        // `Role::KernelIdx` arg indexes, and every operand of every op that does not gather runs the
        // loop unchanged.
...
pub fn emit_sdsc(
    ...
    for (i, v) in views.iter().enumerate() {
        ...
        // `test_gather_1core/sdsc_1.json` carries exactly ONE address for its value tensor. `gathered`
        // says the same thing this crate says everywhere else by scale: this operand does not range over
        // the split axis on its own account.
        //
        // ⛔ AND IT IS READ OFF `gather_pair`, WHICH IS `None` FOR EVERY OP WITH NO `Role::KernelIdx`
        // ARG — so every operand of every non-gathering op is `gathered: false`, which is `per_core_addr`
        // exactly as it was before this door existed.
        ...
        // The walk's extents — HOISTED out of the `device_walk` argument it used to be spelled inside,
        // because a gather's two operands need them by value. The expression is the former argument
        // character for character, still `Span::Swept` over THIS view, and it is still the ONLY thing a
        // non-gathering operand's `maxDimSizes_` is computed from.
        ...
        //
        // ⛔ AND IT IS AN OVERRIDE, NOT A BRANCH. The walk is main's `sl.device_walk(&walk_extents)`
        // first, for every operand of every op; `gather_pair` is `None` unless this op carries a
        // `Role::KernelIdx` arg, so the two replacements below are unreachable without a gather rather
        // than arms a non-gathering operand has to fall through.
        let mut walk = sl.device_walk(&walk_extents);
        if let Some((idx, val)) = gather_pair {
            if idx == i {
                walk = sl.device_walk_index(walk_extents.len())?;
            } else if val == i {
                ...
                walk = sl.device_walk_gather_entry(&walk_extents, entry_dim)?;
            }
        }
        tree.push(AllocNode {
            nodeType_: "allocate",
...
mod indirect_access_baseline {
    ...
    /// `lower_ktir_to_superdsc::scalarmul` builds, restated here because that one is private.
    ...
    /// FNV-1a over `bytes` — a digest with no dependency and no cross-release drift.
    ///
    /// `DefaultHasher` is deliberately NOT used: its output is explicitly not stable across Rust
    /// releases, so a pinned value would rot into a false alarm. FNV-1a is eight lines and fixed
    /// forever, which is the whole requirement for a recorded baseline.
    ...
    /// ⭐⭐⭐ THE REGRESSION THIS WHOLE DOOR WAS HELD BACK FOR: a spread of NON-GATHERING ops emits
    /// descriptor bytes identical to the ones main emits, over every code path the gather touched.
    ///
    /// ⛔ WHY A SPREAD AND NOT ONE OP. The four sites the gather edits inside [`emit_sdsc`] are the
    /// `memOrg_` arm, the input/output ref split, the arrangement-declaration guard and the
    /// `maxDimSizes_` walk — plus `per_core_addr`'s corner fold, reached through a new `gathered`
    /// parameter. Those are per-OPERAND and per-ROLE, so a single-shape assertion exercises one
    /// combination of (rank, stick, split, dtype, residency) and says nothing about the rest. The
    /// shapes below were chosen to move each: a single-core `[1, 4096]` decode, a multi-core `[256,·]`
    /// prefill, the K-split `m=15` lm-head rung, a batched bmm, the padded 49664 logits width, and an
    /// fp32 pointwise island.
    ///
    /// ⛔ AND A DIGEST IS THE RIGHT SHAPE FOR THIS ONE CLAIM, because the claim is about BYTES. The
    /// four named assertions in the tests above say WHICH field moved; this one says whether ANY byte
    /// moved, which is the only statement that covers a field nobody thought to name.
    ///
    /// MEASURED, and that is why the number below is trustworthy rather than merely recorded: the
    /// identical probe was run against a fresh merged-main tree and against this branch, and the two
    /// dumps are `cmp`-identical over 128,965 bytes / 13 descriptors. The digest is of that dump. If
    /// this fails, dump both sides and diff them — the byte offset names the field.
    #[test]
    fn a_non_gathering_spread_emits_main_s_descriptor_bytes() {
        let mut all = String::new();
        // Seven matmul shapes: single/multi-core, batched, K-split, narrow, and the padded logits width.
        for (m, n, k, b) in [
            (384u32, 384u32, 64u32, 16u32),
            (1, 4096, 4096, 1),
            (15, 12800, 4096, 1),
            (32, 4096, 4096, 1),
            (256, 4096, 4096, 1),
            ...
            match matmul_opspec(m, n, k, b, "Tensor0", "Tensor1", "Tensor2") {
                Ok(op) => {
                    let folds = SdscFoldSet::new(op.iter.cores_used());
                    match emit_sdsc("MatMul_0", &op, &folds, None) {
                        Ok(sd) => all.push_str(&format!(
                            "mm {m} {n} {k} {b} OK {}\n",
                            serde_json::to_string(&sd).expect("SdscOp serializes")
                        )),
                        // A refusal is as much a recorded behaviour as an emission: a gate that turned
                        // one into the other would move this digest too.
                        Err(e) => all.push_str(&format!("mm {m} {n} {k} {b} EMITERR {}\n", e.0)),
                    }
                }
                Err(e) => all.push_str(&format!("mm {m} {n} {k} {b} SPECERR {e}\n")),
            }
        }
        // Six pointwise shapes, fp16 and the fp32 island — `per_core_addr`'s 4-byte word length.
        for (rows, cols, df) in [
            (1u32, 4096u32, Df::Fp16),
            (32, 4096, Df::Fp16),
            (256, 4096, Df::Fp16),
            (15, 12800, Df::Fp16),
            (8, 64, Df::Fp16),
            ...
            let mut sym = 0i64;
            let e = assemble_pointwise_broadcast_df(
                "pw_o2", "multiply", rows, cols, &inputs, "t2", df, &mut sym, None,
            );
            all.push_str(&format!(
                "pw {rows} {cols} {df:?} {}\n",
                serde_json::to_string(e.dsc()).expect("Dsc serializes")
            ));
        }

        // ⛔ ANTI-VACUITY: 13 descriptors, and a length that catches a shape silently dropping out of
        // the loops above (an `Err` arm is shorter than an emission by ~10 KB).
        assert_eq!(all.lines().count(), 13, "13 shapes were dumped");
        assert_eq!(
            all.len(),
            128_965,
            "the dump's LENGTH moved, so at least one descriptor changed size"
        );
        assert_eq!(
            fnv1a(all.as_bytes()),
            0x2ae3_717b_3c07_81a7,
            "A NON-GATHERING OP'S DESCRIPTOR BYTES MOVED. The gather door's edits to `emit_sdsc` and \
             `per_core_addr` are gated on `Role::KernelIdx` / `OpSpec::indirect_index_pair`, which no \
             op without an index operand can satisfy — so this digest is main's, measured. Dump this \
             string and the same spread from main, and diff them: the byte offset names the field."
        );
    }
