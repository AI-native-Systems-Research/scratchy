...
pub fn scalarmul(
    ...
    // ⭐ AND WHETHER THE TILE IT MULTIPLIES IS **GATHERED**, which is also the program's statement and
    // not the caller's. `Program::ScalarMul` is the kind of the one COMPUTE op; a Triton embedding is
    // exactly that op over a row tile whose row INDEX is data (`embedding.py`'s
...
pub fn scalarmul_at(
    ...
    if let Some(g) = gather {
        ...
        return crate::emit::assemble_pointwise_broadcast_gather(
            ...
            &inputs,
            0,
            &crate::place::act_name(g.index_tid),
            &rbo(&out_name),
            sym_id_base,
            layout,
        )
