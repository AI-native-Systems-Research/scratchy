...
//! which [`super::opspec::refuse_reduction_core_split`] refuses by name because the dxp scheduler on
//! image `dev-2026_09_11-150524` has no mapping for it (its own `sdsc_bmm_psum` fixtures fail).
//!
...
/// SAME tiler the emission runs, on the SAME dims, with the SAME splitter.
///
...
pub fn plan_k_trips<DF: DataFormat>(
    ...
    let splitter = matmul_split_map_for(form);
    ...
    for t in 1..=k_sticks {
        ...
        // The tiler is the AUTHORITY on the fit: `Err` means not even `out`-tiled to one stick plus
        // the whole reduction-core ladder placed it, and an `in > 1` plan means it was placed only
        // by a split the dxp image cannot schedule. Either way this trip count is not the answer.
    ...
    Err(format!(
        "matmul {m}x{n}x{k}: NO K-trip count fits LX without splitting the reduction axis across \
         ...
         refused outright (the `out` time-tile bottoms out at one stick) or was placed only by an \
         `in > 1` reduction-core split, which dxp image `dev-2026_09_11-150524` cannot schedule \
         (`refuse_reduction_core_split`). The per-trip `in` splits the ladder reported, in trip-count \
...
mod tests {
    ...
    /// `in: 2` reduction-core split, which is the descriptor dxp cannot schedule.
    #[test]
    fn granite_down_proj_at_64_rows_needs_two_k_trips() {
        ...
        .expect("the LX repair places it, just not schedulably");
        assert!(
            whole.plan.split_of(InAxis::NAME) > 1,
            "the control must be the unschedulable case, got in={}",
