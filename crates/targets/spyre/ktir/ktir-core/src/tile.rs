...
pub enum TileStorage {
    ...
    Bool(Rc<[u8]>),
    /// `float8_e4m3fn` bit patterns, one byte per element.
    ///
    /// Stored PACKED, as the device holds it: an fp8 weight IS one byte per element, not a
    /// compressed f16, so widening happens on read rather than at load.
    Fp8E4m3(Rc<[u8]>),
}
...
impl TileStorage {
    ...
    fn as_f32(&self) -> Cow<'_, [f32]> {
        match self {
            ...
            TileStorage::Fp8E4m3(s) => {
                Cow::Owned(s.iter().map(|&b| crate::codec::e4m3_to_f32(b)).collect())
            }
        }
    }
    ...
    fn encode(data: &[f32], dtype: DType) -> TileStorage {
        match dtype {
            ...
            DType::Fp8E4m3 => {
                TileStorage::Fp8E4m3(data.iter().map(|&x| crate::codec::f32_to_e4m3(x)).collect())
            }
...
impl Tile {
    ...
    /// Widen `out.len()` elements starting at `off` DIRECTLY into `out`.
    ///
    /// ⛔ WHY NOT `as_f32()[off..off + n]`. For any storage that is not already f32 that call
    /// widens the WHOLE tile into a fresh `Vec` — which the caller then copies again into its own
    /// buffer. A GEMM's activation operand is exactly that shape of use: two allocations and two
    /// passes to move one slice. Returns `false` when the range is out of bounds, so a caller can
    /// report it rather than silently short-read.
    pub fn write_f32_into(&self, off: usize, out: &mut [f32]) -> bool {
        if off + out.len() > self.len() {
            return false;
        }
        match &self.data {
            TileStorage::F32(s) => out.copy_from_slice(&s[off..off + out.len()]),
            TileStorage::F16(s) => {
                let src = &s[off..off + out.len()];
                for (o, &u) in out.iter_mut().zip(src) {
                    *o = crate::codec::f16_bits_to_f32(u);
                }
            }
            // The remaining storages are not on any hot path; go through the shared widening.
            _ => {
                let all = self.data.as_f32();
                out.copy_from_slice(&all[off..off + out.len()]);
            }
        }
        true
    }
    ...
    pub fn data_ptr(&self) -> usize {
        match &self.data {
            ...
            TileStorage::Fp8E4m3(s) => Rc::as_ptr(s) as *const u8 as usize,
        }
    }
}
