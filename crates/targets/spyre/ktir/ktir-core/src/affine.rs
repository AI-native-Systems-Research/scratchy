...
/// symbolic-bound layer constructs.
///
/// ⭐ CHILDREN ARE BORROWS, NOT `Rc`. An `Rc` is not `Sync`, so an expression holding one cannot
/// appear in a `static` — and a baked program IS a static (`inventory::submit!` expands to
/// `static __INVENTORY: Node = Node { value: &{ .. } }`). Borrowing keeps clones as cheap as the
/// refcount bump was, since the tree is immutable once built, and makes a whole function
/// const-constructible. `consttest.rs` is what checks that on every build.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum AffineExpr<'a> {
    Dim(usize),
    Sym(usize),
    Const(i64),
    ...
    Ref(&'a str),
    Add(&'a AffineExpr<'a>, &'a AffineExpr<'a>),
    Sub(&'a AffineExpr<'a>, &'a AffineExpr<'a>),
    Neg(&'a AffineExpr<'a>),
    Mul(&'a AffineExpr<'a>, &'a AffineExpr<'a>),
    FloorDiv(&'a AffineExpr<'a>, &'a AffineExpr<'a>),
    Mod(&'a AffineExpr<'a>, &'a AffineExpr<'a>),
    Max(&'a AffineExpr<'a>, &'a AffineExpr<'a>),
    Min(&'a AffineExpr<'a>, &'a AffineExpr<'a>),
}
...
impl<'a> AffineMap<'a> {
    ...
    /// The identity's expressions are `Dim(0..n)`, so a caller with an arena builds them there;
    /// `IDENTITY_DIMS` covers the ranks the emitter actually uses without one.
    pub fn identity_in(exprs: &'a [AffineExpr<'a>]) -> AffineMap<'a> {
        AffineMap {
            num_dims: exprs.len(),
            num_syms: 0,
            exprs,
...
impl From<i64> for Bound<'_> {
    fn from(v: i64) -> Self {
        Bound::Concrete(v)
    }
}

impl<'a> From<&'a AffineExpr<'a>> for Bound<'a> {
    fn from(e: &'a AffineExpr<'a>) -> Self {
        // Fold a bare constant node into the concrete leaf so structural fast paths keep working —
        // mirrors `("const", k)` never appearing alone. The symbolic case BORROWS: an owned node
        // would need somewhere to live, and the caller's arena is where that is.
        match e {
            AffineExpr::Const(c) => Bound::Concrete(*c),
            other => Bound::Symbolic(other),
        }
    }
}
...
/// `parser_ast.sym_add`.
pub fn sym_add<'a>(ar: &'a Arena, a: &Bound<'a>, b: &Bound<'a>) -> Bound<'a> {
    match (a, b) {
        (Bound::Concrete(x), Bound::Concrete(y)) => Bound::Concrete(x + y),
        (Bound::Concrete(0), _) => *b,
        (_, Bound::Concrete(0)) => *a,
        _ => Bound::Symbolic(ar.expr(AffineExpr::Add(
            ar.expr(bound_to_node(a)),
            ar.expr(bound_to_node(b)),
        ))),
    }
}
...
pub fn sym_neg<'a>(ar: &'a Arena, a: &Bound<'a>) -> Bound<'a> {
    match a {
        Bound::Concrete(c) => Bound::Concrete(-c),
        Bound::Symbolic(node) => match node {
            AffineExpr::Neg(inner) => Bound::from(*inner),
            other => Bound::Symbolic(ar.expr(AffineExpr::Neg(ar.expr(**other)))),
        },
    }
}
...
/// `parser_ast.sym_max`.
pub fn sym_max<'a>(ar: &'a Arena, a: &Bound<'a>, b: &Bound<'a>) -> Bound<'a> {
    sym_minmax(ar, a, b, true)
}

/// Build `min(a, b)`; mirror of [`sym_max`] (`parser_ast.sym_min`).
pub fn sym_min<'a>(ar: &'a Arena, a: &Bound<'a>, b: &Bound<'a>) -> Bound<'a> {
    sym_minmax(ar, a, b, false)
}

fn sym_minmax<'a>(ar: &'a Arena, a: &Bound<'a>, b: &Bound<'a>, is_max: bool) -> Bound<'a> {
    ...
    if let (Bound::Symbolic(na), Bound::Symbolic(nb)) = (a, b)
        && let (AffineExpr::Sym(i), AffineExpr::Sym(j)) = (na, nb)
        && i == j
    {
        return *a;
    }
    let an = ar.expr(bound_to_node(a));
    let bn = ar.expr(bound_to_node(b));
    let node = if is_max {
        AffineExpr::Max(an, bn)
    } else {
        AffineExpr::Min(an, bn)
    };
    Bound::Symbolic(ar.expr(node))
}

/// Lift a [`Bound`] to an [`AffineExpr`] node (wraps concrete ints in `Const`),
/// matching `("const", a) if isinstance(a, int) else a`.
fn bound_to_node<'a>(b: &Bound<'a>) -> AffineExpr<'a> {
    match b {
        Bound::Concrete(c) => AffineExpr::Const(*c),
        Bound::Symbolic(node) => **node,
    }
}
...
impl<'a> AffineSet<'a> {
    ...
    pub fn intersect(&self, ar: &'a Arena, other: &AffineSet<'a>) -> AffineSet<'a> {
        assert_eq!(
        ...
        constraints.extend(other.constraints.iter().copied());
        AffineSet {
...
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct SymBoxSet<'a> {
    pub lo: Vec<Bound<'a>>, // inclusive
    pub hi: Vec<Bound<'a>>, // exclusive
    all_concrete: bool,
}
...
impl<'a> SymBoxSet<'a> {
    ...
    pub fn new(lo: Vec<Bound<'a>>, hi: Vec<Bound<'a>>) -> SymBoxSet<'a> {
        assert_eq!(
            lo.len(),
            hi.len(),
    ...
    /// `BoxSet.specialize`.
    pub fn specialize(&self, symbols: &[i64]) -> SymBoxSet<'a> {
    ...
    pub fn translate(&self, ar: &'a Arena, offset: &[Bound<'a>]) -> SymBoxSet<'a> {
        assert_eq!(
            offset.len(),
            self.n_dims(),
        ...
        SymBoxSet::new(
            (0..self.n_dims())
                .map(|d| sym_add(ar, &self.lo[d], &offset[d]))
                .collect(),
            (0..self.n_dims())
                .map(|d| sym_add(ar, &self.hi[d], &offset[d]))
                .collect(),
        )
    }
    ...
    pub fn intersect(&self, ar: &'a Arena, other: &SymBoxSet<'a>) -> SymBoxSet<'a> {
        assert_eq!(
            other.n_dims(),
            self.n_dims(),
        ...
        SymBoxSet::new(
            (0..self.n_dims())
                .map(|d| sym_max(ar, &self.lo[d], &other.lo[d]))
                .collect(),
            (0..self.n_dims())
                .map(|d| sym_min(ar, &self.hi[d], &other.hi[d]))
                .collect(),
        )
    }
    ...
    pub fn try_from_affine_set(ar: &'a Arena, aset: &AffineSet<'a>) -> Option<SymBoxSet<'a>> {
        let n = aset.num_dims;
        let n_syms = aset.num_syms;
        let mut los: Vec<Option<Bound<'a>>> = vec![None; n];
        let mut his: Vec<Option<Bound<'a>>> = vec![None; n];
        ...
        for c in aset.constraints {
            ...
            let sym_term = build_sym_term(ar, &sym_coeffs, const_);
            if is_eq {
                ...
                let pin_hi = sym_add(ar, &pin, &Bound::Concrete(1));
                los[i] = Some(match &los[i] {
                    None => pin,
                    Some(cur) => sym_max(ar, cur, &pin),
                });
                his[i] = Some(match &his[i] {
                    None => pin_hi,
                    Some(cur) => sym_min(ar, cur, &pin_hi),
                });
            } else if k == 1 {
                // d_i + k(syms) >= 0  ->  d_i >= -k(syms)
                let candidate = sym_neg(ar, &sym_term);
                los[i] = Some(match &los[i] {
                    None => candidate,
                    Some(cur) => sym_max(ar, cur, &candidate),
                });
            } else {
                // -d_i + k(syms) >= 0  ->  d_i <= k(syms)  ->  hi excl = k+1
                let candidate = sym_add(ar, &sym_term, &Bound::Concrete(1));
                his[i] = Some(match &his[i] {
                    None => candidate,
                    Some(cur) => sym_min(ar, cur, &candidate),
                });
            }
        }
...
fn build_sym_term<'a>(ar: &'a Arena, sym_coeffs: &[i64], const_: i64) -> Bound<'a> {
    let mut expr = Bound::Concrete(const_);
    for (j, &c) in sym_coeffs.iter().enumerate() {
        if c == 0 {
            continue;
        }
        let sym = ar.expr(AffineExpr::Sym(j));
        let term: Bound<'a> = if c == -1 {
            sym_neg(ar, &Bound::Symbolic(sym))
        } else if c != 1 {
            Bound::Symbolic(ar.expr(AffineExpr::Mul(ar.expr(AffineExpr::Const(c)), sym)))
        } else {
            Bound::Symbolic(sym)
        };
        expr = sym_add(ar, &expr, &term);
    }
    expr
}
...
mod tests {
    ...
    /// The process arena — the same storage a real build's borrows come from, and the only one
    /// whose life outlives every borrow a test hands around.
    fn ar() -> &'static Arena {
        Arena::global()
    }

    /// Lift `e` into the arena and hand back the borrow a child slot holds — what `Rc::new` did
    /// before children became borrows (an `Rc` is not `Sync`, so it cannot appear in a baked
    /// `static`).
    fn node(e: AffineExpr<'static>) -> &'static AffineExpr<'static> {
        ar().expr(e)
    }

    // Helpers for building expressions concisely.
    fn dim(i: usize) -> AffineExpr<'static> {
        AffineExpr::Dim(i)
    }
    fn sym(i: usize) -> AffineExpr<'static> {
        AffineExpr::Sym(i)
    }
    fn cst(c: i64) -> AffineExpr<'static> {
        AffineExpr::Const(c)
    }
    fn add(a: AffineExpr<'static>, b: AffineExpr<'static>) -> AffineExpr<'static> {
        AffineExpr::Add(node(a), node(b))
    }
    fn sub(a: AffineExpr<'static>, b: AffineExpr<'static>) -> AffineExpr<'static> {
        AffineExpr::Sub(node(a), node(b))
    }
    fn neg(a: AffineExpr<'static>) -> AffineExpr<'static> {
        AffineExpr::Neg(node(a))
    }
    fn mul(c: i64, a: AffineExpr<'static>) -> AffineExpr<'static> {
        AffineExpr::Mul(node(cst(c)), node(a))
    }

    /// The identity map of rank `n` — what `AffineMap::identity(n)` built before the map came to
    /// hold a BORROWED expression slice. `identity_in`'s own doc says a caller with an arena
    /// builds the `Dim(0..n)` expressions there, which is exactly this.
    fn identity(n: usize) -> AffineMap<'static> {
        AffineMap::identity_in(ar().exprs((0..n).map(dim).collect()))
    }
    ...
    fn map_eval_and_identity() {
        ...
        let m = AffineMap {
            ...
            num_syms: 1,
            exprs: ar().exprs(vec![
                AffineExpr::Add(node(AffineExpr::Dim(0)), node(AffineExpr::Sym(0))),
                AffineExpr::Mul(node(AffineExpr::Dim(1)), node(AffineExpr::Const(2))),
            ]),
        };
        assert_eq!(m.eval(&[5, 7], &[10]), vec![15, 14]);
        assert_eq!(identity(3).eval(&[1, 2, 3], &[]), vec![1, 2, 3]);
    }

    #[test]
    fn euclidean_floordiv_and_mod() {
        let fd = AffineExpr::FloorDiv(node(AffineExpr::Dim(0)), node(AffineExpr::Const(4)));
        let m = AffineExpr::Mod(node(AffineExpr::Dim(0)), node(AffineExpr::Const(4)));
    ...
    fn expr_sub_neg_max_min_eval() {
        ...
        // max(d0, d1), min(d0, d1)
        let mx = AffineExpr::Max(node(dim(0)), node(dim(1)));
        let mn = AffineExpr::Min(node(dim(0)), node(dim(1)));
    ...
    fn affine_set_membership() {
        ...
        let set = AffineSet {
            num_dims: 1,
            num_syms: 0,
            constraints: ar().constraints(vec![
                Constraint {
                ...
                Constraint {
                    expr: AffineExpr::Add(
                        node(AffineExpr::Const(7)),
                        node(AffineExpr::Mul(
                            node(AffineExpr::Const(-1)),
                            node(AffineExpr::Dim(0)),
                        )),
                    ),
                    kind: ConstraintKind::GreaterEq,
                },
    ...
    #[test]
    fn map_is_identity() {
        assert!(identity(3).is_identity());
        // (d0, d1) -> (d1, d0) is a permutation, not identity.
        let swap = AffineMap {
            num_dims: 2,
            num_syms: 0,
            exprs: ar().exprs(vec![dim(1), dim(0)]),
        };
        ...
        let shifted = AffineMap {
            num_dims: 1,
            num_syms: 0,
            exprs: ar().exprs(vec![add(dim(0), cst(1))]),
        };
        ...
        let folded = AffineMap {
            num_dims: 2,
            num_syms: 0,
            exprs: ar().exprs(vec![sub(add(dim(0), dim(1)), dim(1)), dim(1)]),
        };
    ...
    fn map_is_permutation() {
        let perm = AffineMap {
            num_dims: 3,
            num_syms: 0,
            exprs: ar().exprs(vec![dim(2), dim(0), dim(1)]),
        };
        ...
        let dup = AffineMap {
            num_dims: 2,
            num_syms: 0,
            exprs: ar().exprs(vec![dim(0), dim(0)]),
        };
        ...
        let scaled = AffineMap {
            num_dims: 1,
            num_syms: 0,
            exprs: ar().exprs(vec![mul(2, dim(0))]),
        };
        assert!(!scaled.is_permutation());
        // Identity is also a permutation.
        assert!(identity(2).is_permutation());
    }
    ...
    fn affine_set_enumerate_and_full() {
        ...
        let set = AffineSet {
            num_dims: 2,
            num_syms: 0,
            constraints: ar().constraints(vec![Constraint {
    ...
    fn symbox_concrete_basics() {
        ...
        let t = b.translate(ar(), &[Bound::Concrete(1), Bound::Concrete(0)]);
    ...
    fn symbox_empty_and_intersect() {
        ...
        assert!(d.intersect(ar(), &e).is_empty(&[]));
    }

    #[test]
    fn symbox_symbolic_specialize() {
        // lo = [s0], hi = [s0 + 2]  — a width-2 window starting at s0.
        let lo = Bound::Symbolic(node(sym(0)));
        let hi = sym_add(ar(), &Bound::Symbolic(node(sym(0))), &Bound::Concrete(2));
    ...
    fn sym_helpers_fold() {
        ...
        assert_eq!(
            sym_add(ar(), &Bound::Concrete(2), &Bound::Concrete(3)),
            Bound::Concrete(5)
        );
        // additive identity
        assert_eq!(
            sym_add(ar(), &Bound::Concrete(0), &Bound::Symbolic(node(sym(0)))),
            Bound::Symbolic(node(sym(0)))
        );
        // double-negation collapse
        let s = Bound::Symbolic(node(sym(1)));
        assert_eq!(sym_neg(ar(), &sym_neg(ar(), &s)), s);
        // concrete min/max fold
        assert_eq!(
            sym_max(ar(), &Bound::Concrete(2), &Bound::Concrete(7)),
            Bound::Concrete(7)
        );
        assert_eq!(
            sym_min(ar(), &Bound::Concrete(2), &Bound::Concrete(7)),
            Bound::Concrete(2)
        );
        // idempotent on identical symbol refs
        let sk = Bound::Symbolic(node(sym(3)));
        assert_eq!(sym_max(ar(), &sk, &sk), sk);
    }
    ...
    fn lower_concrete_axis_aligned_set() {
        ...
        let b = SymBoxSet::try_from_affine_set(ar(), &set).expect("should lower");
    ...
    fn lower_rejects_non_axis_aligned() {
        ...
        assert!(SymBoxSet::try_from_affine_set(ar(), &set).is_none());
        ...
        assert!(SymBoxSet::try_from_affine_set(ar(), &set2).is_none());
    }
    ...
    fn lower_equality_pins_both_sides() {
        ...
        let b = SymBoxSet::try_from_affine_set(ar(), &set).expect("equality lowers");
    ...
    fn lower_symbolic_bounds() {
        ...
        let b = SymBoxSet::try_from_affine_set(ar(), &set).expect("symbolic lowers");
        assert!(!b.is_concrete());
