...
pub enum AttrKey {
    ...
    //
    // MEASURED at `SCRATCHY_SUPERDSC_GROUP_SIZE=512`: `DimSubs` at its alphabetical position (between
    // `DimMap0` and `Dimensions`) moved a 134-bundle set from `ac71651adc80` to `a8d5f0a7e848` with
    // 0 of 134 fingerprints in common — every bundle renamed, not one descriptor byte changed. ⛔ BOTH
    // of those literals PREDATE #87, which is a breaking change and moved the emitted set, so neither
    // is a current baseline; they are kept only because the 0-in-common EFFECT is what they establish.
    //
    // RE-MEASURED against merged main (granite-3.1-2b-instruct, `-Fspyre-hw,serve`, two trees built
    // from scratch with no seeded `target/`): appended here, the set is main's own — 134 of 134
    // fingerprints IN COMMON, set hash `35d8c84a33d3` on both sides, `diff -r` over the two
    // `superdsc-code/` trees empty, and a per-tree content digest of `578e1bb0243c7eb9` over 1155 files
    // each. In-common is the load-bearing number, not the set-hash match: a hash-INPUT change lands
    // 0 in common while changing no byte, and an emission change moves a handful.
...
///
/// The four pins below are the recorded positions. `Dimensions == 11` is the load-bearing one: it is
/// the first variant an alphabetical `dim_*` insertion displaces, so it is the pin that FAILS on the
/// mistake this module exists to catch — it read 12, not 11, in the arrangement whose 134-bundle set
/// came out `a8d5f0a7e848` with 0 of 134 fingerprints in common.
#[cfg(test)]
