...
use scratchy_subtile::sdsc_abstract::{MatK, MatM, MatN, MatY, PhysM};
...
pub fn matmul_opspec_split<DF: DataFormat, S>(
    ...
    batch_strides: Option<scratchy_subtile::sdsc_abstract::BatchStrides>,
    splitter: S,
) -> Result<OpSpec, String>
where
    S: FnOnce(
    ...
    let args = if batch <= 1 {
        ...
        if let Some(want) = batch_strides {
            ...
            let check = |regime_stride: u32,
                         have: scratchy_subtile::addr::AxisStride,
                         role: &str,
                         axis: &str,
                         ext: u32|
             -> Result<(), String> {
...
pub fn matmul_opspec_batched(
    ...
    matmul_opspec_batched_off(
        MatM::of_query_rows(scratchy_subtile::sdsc_abstract::QueryRowCount::of_mq(m)),
        MatN::of_out_features(n),
        MatK::of_in_features(k),
        MatY::of_heads(batch),
        a_name,
