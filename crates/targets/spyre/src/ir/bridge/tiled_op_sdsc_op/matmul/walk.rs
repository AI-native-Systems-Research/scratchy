...
//!
//! [`StickLayout::off_view`]: scratchy_subtile::sdsc_abstract::StickLayout::off_view

use std::marker::PhantomData;

use scratchy_subtile::sdsc_abstract::QueryRowCount;
...
impl SharedKernelBmmForm {
    ...
    /// ([`scratchy_subtile::sdsc_abstract::BatchStrides`]):
    ...
    pub fn of_score_leg(
        ...
        mq: QueryRowCount,
        slabs: scratchy_subtile::addr::Slabs,
    ) -> Self {
        if slabs.get() > 1 {
            SharedKernelBmmForm(SharedKernelBmmRegime::Requests(HeadOutermostRequests(())))
        } else {
    ...
    ///
    /// The write's `y` axis covers `(kv head, slab)` PAIRS ([`scratchy_subtile::sdsc_abstract::KvPlanes`]), whose
...
mod site_sealed {
    ...
    impl Sealed for crate::lower_subtile_tape_to_superdsc::bmm_site::TapeLoweringSite {}
}
...
impl NonAttnBmmSite for crate::lower_subtile_tape_to_superdsc::bmm_site::TapeLoweringSite {}
...
pub(crate) trait RungRegime: sealed::Sealed + Copy {
    ...
    /// ([`scratchy_subtile::sdsc_abstract::BatchStrides`]) and refuses on mismatch.
