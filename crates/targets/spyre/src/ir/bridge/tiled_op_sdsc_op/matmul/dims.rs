...
use scratchy_subtile::superdsc_opspec::{DataFormat, Df};
...
pub(crate) fn matmul_dims<DF: DataFormat>(
    m: u32,
    n_ext: &scratchy_subtile::superdsc_opspec::StickExtent<DF>,
    k_ext: &scratchy_subtile::superdsc_opspec::StickExtent<DF>,
    batch: u32,
) -> Vec<scratchy_subtile::superdsc_opspec::ItDim> {
    ...
    if batch > 1 {
        dims.push(scratchy_subtile::superdsc_opspec::ItDim {
            name: YAxis::NAME,
            size: batch,
            is_reduction: false,
            is_stick: false,
...
// The matmul cost-model splitter as a [`scratchy_subtile::superdsc_opspec::WorkPlan`] splitter: maps the
...
pub(crate) fn set_split_mb_forbidden(v: bool) -> bool {
...
pub(crate) fn matmul_split_map_gated<const FORBID_MB_SPLIT: bool>(
    dims: &[scratchy_subtile::superdsc_opspec::ItDim],
    max_cores: u32,
...
fn matmul_split_map_inner(
    ...
    use crate::lower_subtile_tape_to_superdsc::{
        FP16_ELEMS_PER_STICK, core_split, matmul_cost_split, matmul_split_plan,
    };
