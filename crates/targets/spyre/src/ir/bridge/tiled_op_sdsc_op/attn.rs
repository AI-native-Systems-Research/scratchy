...
use crate::lower_subtile_tape_to_superdsc::{
    BundleLayout, EmittedOp, In, assemble_pointwise_broadcast_off, assemble_restickify_kt_2d, fl,
    rb,
};
use scratchy_subtile::addr::{Head, Idx, Nest};
...
use scratchy_subtile::superdsc_error::SuperDscError;
use scratchy_subtile::superdsc_opspec::{DataFormat, Df, Fp16};
...
pub(crate) const STICK_WIDE_BUFS: [crate::bundle_code::SynthRole; 11] = {
...
pub(crate) const HEAD_WIDE_BUFS: [crate::bundle_code::SynthRole; 2] = {
    use crate::bundle_code::SynthRole as R;
    [R::RunO, R::OTmp]
};
...
struct BlockNests {
    ...
    mq: scratchy_subtile::sdsc_abstract::QueryRowCount,
...
impl BlockRows {
    ...
    fn regime(self) -> scratchy_subtile::sdsc_abstract::FoldRowRegime {
        match self {
            BlockRows::WholeBatch(_) => scratchy_subtile::sdsc_abstract::FoldRowRegime::WholeBatch,
            BlockRows::PerRequest(_) => scratchy_subtile::sdsc_abstract::FoldRowRegime::PerRequest,
        }
    }
}
impl BlockNests {
    ...
    fn head_row(&self, h: u32) -> Idx<scratchy_subtile::addr::Row> {
        let row = match self.rows {
            ...
            BlockRows::PerRequest(_) => {
                scratchy_subtile::sdsc_abstract::RequestHeadRow::of(self.req, h, self.heads).get()
            }
            ...
            BlockRows::WholeBatch(_) => {
                scratchy_subtile::sdsc_abstract::HeadRequestRow::of(h, 0, self.mq.get()).get()
            }
        };
        Idx::<scratchy_subtile::addr::Row>::n(row)
    }
    fn slabs(&self) -> u32 {
        scratchy_subtile::addr::Shape::<0, 0, 0, 0>::slabs_of(
            self.hd,
            scratchy_subtile::superdsc_opspec::Df::Fp16,
        )
        .get()
    }
    ...
    fn head_major(&self, h: u32, s: u32) -> scratchy_subtile::addr::DevOff {
        scratchy_subtile::addr::Nest::new(
            &["row", "feat"],
    ...
    fn score_rows(
        ...
        h: u32,
        width: BlockCols,
    ) -> scratchy_subtile::sdsc_abstract::OperandPlacement {
        match self.rows {
            BlockRows::PerRequest(rows) => {
                scratchy_subtile::sdsc_abstract::OperandPlacement::of_request_major_rows(
                    self.req, h, rows, width,
                )
            }
            BlockRows::WholeBatch(rows) => {
                scratchy_subtile::sdsc_abstract::OperandPlacement::of_head_major_rows(
                    h, self.mq, rows, width,
                )
            }
        }
    }
    ...
    fn token_stream(&self, h: u32, s: u32) -> scratchy_subtile::sdsc_abstract::OperandPlacement {
        scratchy_subtile::sdsc_abstract::OperandPlacement::of_token_stream(
            self.mq, self.heads, self.hd, h, s,
        )
    }
...
impl ScoreArm {
    ...
    /// [`scratchy_subtile::sdsc_abstract::BatchStrides`] carrying `a_pitch`/`o_pitch` — and, the half that was
...
fn per_core_blocks(e: &EmittedOp) -> Vec<(String, bool, u64, u64)> {
    ...
    let mut out = Vec::new();
    for dsc in e.op.dscs_.iter().flat_map(|m| m.values()) {
...
fn assemble_attn_block(
    ...
    kt_stride: scratchy_subtile::sdsc_abstract::KtKernelPitch,
    ...
    kt_off_fn: impl Fn(
        scratchy_subtile::sdsc_abstract::QueryHead,
        u32,
    ) -> scratchy_subtile::addr::DevOff,
    v_kernel: &str,
    v_stride: u32,
    ...
    v_off_fn: impl Fn(scratchy_subtile::sdsc_abstract::QueryHead, u32) -> scratchy_subtile::addr::DevOff,
    ...
    width: scratchy_subtile::sdsc_abstract::BlockCols,
    mask: &str,
    mask_off: scratchy_subtile::addr::DevOff,
    mask_bcast: bool,
    ...
    let broadcast_dims: &[(&str, scratchy_subtile::superdsc_opspec::Scale)] = if mask_bcast {
        &[("mb", scratchy_subtile::superdsc_opspec::Scale::RedNonStick)]
    } else {
        &[]
    };
        ...
        .filter(|&nkvh_b| ScoreArm::choose(nqh, nkvh_b, nslab) == ScoreArm::SlabSplitBatched)
    {
        ...
        for kvh in scratchy_subtile::sdsc_abstract::KvHead::all(nkvh_nz) {
            ...
            let qh0 = kvh.group_first_query(scratchy_subtile::addr::Gqa::new(gqa));
            let h0 = qh0.get();
            ...
            for s in 0..nslab {
                let (epi_h, epi_off, epi_bd, epi_bc): (
                    &Stk<FlatTag>,
                    scratchy_subtile::addr::DevOff,
                    &[(&str, scratchy_subtile::superdsc_opspec::Scale)],
                    bool,
                ) = if s == 0 {
                ...
                ops.push(if per_request {
                    assemble_matmul_off_phys_m_with_epilogue(
                        ...
                        epi_h,
                        epi_off,
                        scratchy_subtile::superdsc_opspec::EpilogueOpFunc::StridedAdd,
                        epi_bd,
                        epi_bc,
                        sym_id_base,
                        layout,
        ...
        for h in scratchy_subtile::sdsc_abstract::QueryHead::all(nqh_nz) {
            let hi = h.get();
                ...
                ops.push(if per_request {
                    assemble_matmul_off_phys_m_maybe_epilogue(
                        ...
                        MatY::of_gqa_group(
                            gqa,
                            nests.score_rows(h0, width),
                            scratchy_subtile::sdsc_abstract::OperandPlacement::of_head_major_accum(
                                rows.swept(),
                                hd,
                                mq,
                                h0,
...
pub fn assemble_attn<const NQH: u32, const NKVH: u32, const HD: u32>(
    ...
    bundle_rows: scratchy_subtile::sdsc_abstract::AttnBundleRows<NQH>,
    cap: u32,
    active_cap: u32,
    qs: &str,
    new_k_scaled: &str,
    ...
    let new_kt = crate::lower_subtile_tape_to_superdsc::syn(layout, out_id.synth(R::NewKt));
    ...
    let nslab = scratchy_subtile::addr::Shape::<0, 0, 0, 0>::slabs_of(
        hd,
        scratchy_subtile::superdsc_opspec::Df::Fp16,
    )
    .get();
        ...
        for rw in mq_pad_t.row_windows() {
            ...
            for sl in 0..nslab {
                ops.push(assemble_restickify_kt_2d(
                    ...
                    .block(kvh)
                    .at(Idx::<scratchy_subtile::addr::Feat>::n(
                        FeatIdx::of_slab(sl).get(),
                    ))
                    .slab(rw.index())
                    .dev(),
                    sym_id_base,
    ...
    let score_form = SharedKernelBmmForm::of_score_leg(
        rows_are_requests,
        width.mq(),
        scratchy_subtile::addr::Shape::<0, 0, 0, 0>::slabs_of(hd, Df::Fp16),
    );
        ...
        for rw in mq_pad_t.row_windows() {
            ...
            assemble_attn_block(
                ...
                |h, sl| {
                    ...
                    let blk = scratchy_subtile::addr::Nest::new(
                        &["feat", "slot"],
                        &[hd, mq_pad_t.slots().slot_axis_extent()],
                        Df::Fp16,
                    );
                    let kvh =
                        scratchy_subtile::sdsc_abstract::gqa_kv_head(hi as usize, gqa as usize)
                            as u32;
                    ...
                    blk.block(kvh)
                        .slab(rw.index())
                        .slab_of::<scratchy_subtile::addr::Feat>(sl)
                        .dev()
                },
    ...
    for w in SlotWindow::sweep(scratchy_subtile::sdsc_abstract::SlotCount::new(active_cap)) {
        let b = w.index();
        ...
        if std::env::var_os("SCRATCHY_SDSC_FOLD_TRACE").is_some() {
            ...
            let kt_block = scratchy_subtile::sdsc_abstract::KvCoord::block(
                scratchy_subtile::sdsc_abstract::KvPlane::Kt,
                scratchy_subtile::sdsc_abstract::KvHead::of_query(
                    scratchy_subtile::sdsc_abstract::QueryHead::FIRST,
                    scratchy_subtile::addr::Gqa::new(gqa),
                ),
            );
            let at = |win: SlotWindow| -> u32 { pool.addr(kt_block.at_slot(win.first_slot())) };
            let nb =
                SlotWindow::count_in(scratchy_subtile::sdsc_abstract::SlotCount::new(active_cap))
                    .get();
            eprintln!(
        ...
        assemble_attn_block(
            ...
            kct,
            scratchy_subtile::sdsc_abstract::PagedKvPool::KT_KERNEL_PITCH,
            ...
            |h, sl| {
                use scratchy_subtile::sdsc_abstract::{KvCoord, KvPlane};
                let kvh = scratchy_subtile::sdsc_abstract::KvHead::of_query(
                    h,
                    scratchy_subtile::addr::Gqa::new(gqa),
                );
                scratchy_subtile::addr::DevOff::from_view_step(
                    pool.addr(
            ...
            if rows_are_requests {
                ...
                scratchy_subtile::addr::DevOff::of_mask_corner(
                    mask_shape.slab_elems(0, w.mask_slab()),
                )
            } else {
