...
//! core-split proof (already `m`-agnostic in [`WorkPlan`]/[`time_tile_for_lx`](scratchy_subtile::superdsc_opspec))
...
use scratchy_subtile::superdsc_opspec::{Df, FP16_BYTES, ItDim, MaxCores, WorkPlan};
...
/// now a thin wrapper calling this (one implementation, not two proven-equal copies). `pub(crate)` so
...
pub(crate) fn matmul_lx_resident_generic(
    plan: &WorkPlan,
    out_per_time: u32,
    operand_df: Df,
) -> u64 {
...
/// pointwise_lx_resident` is now a thin wrapper calling this. `pub(crate)` for the same reason as
/// [`matmul_lx_resident_generic`].
pub(crate) fn pointwise_lx_resident_generic(
    plan: &WorkPlan,
    n_operands: u64,
    out_per_time: u32,
    df: Df,
