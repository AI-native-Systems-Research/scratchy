#!/bin/bash
# ══════════════════════════════════════════════════════════════════════════════════════════════
# SENTIENT IN-PLACE PASS CAMPAIGN DRIVER  (the D29-D75 span, 656 units, 52 passes)
#
...
# ══════════════════════════════════════════════════════════════════════════════════════════════
ROOT=/Users/nickm/git/scratchy/.claude/worktrees/senpass
CAMPDIR=$ROOT/crustify/campaigns/senpass
LOGDIR=$CAMPDIR/logs
TRACE=$LOGDIR/driver.trace
UNITS=$ROOT/crustify-senpass/UNITS.tsv
# ⛔ PROGRESS IS COUNTED UNDER THE CAMPAIGN'S OWN OUTPUT DIRECTORY. Session refs and eNNN_ unit
# names are both repo-wide; a branch-based or crate-wide count returns another campaign's number.
OUT=$ROOT/crates/compiler/deeptools/src/transform/sentient
BRANCH=sentient-passes-campaign
PARALLEL=6
...
# its OWN target/ by default. That is where bridge 2's 10G went (dozens of leftover agent worktrees),
# and with 19G free on this host six parallel agents would trip the 12G disk guard within the first
# stage. Exported here so the crustify child and every agent it spawns inherit it.
#
...
# tight. Only worktrees whose branch is already an ancestor of HEAD are removed.
prune() {
  n=0
...
say "CAMPAIGN START: 656 units, 52 passes, levels 0..12, --parallel-max $PARALLEL"
count
...
gate sc2
stage sc3-levels2-3-pass-bodies/port.json         port   sc3-port
stage sc3-levels2-3-pass-bodies/review.json       review sc3-review
gate sc3
stage sc4-levels4-6-drivers/port.json             port   sc4-port
stage sc4-levels4-6-drivers/review.json           review sc4-review
gate sc4
stage sc5-levels7-plus-roots/port.json            port   sc5-port
stage sc5-levels7-plus-roots/review.json          review sc5-review
gate sc5
